...
static void udp_server_task(void *pvParameters)
{
    ...
    StreamBufferHandle_t network_stream_buf = (StreamBufferHandle_t) pvParameters;
    ...
    while (1) {
        ...
        iov.iov_base = udp_server_buff;
        iov.iov_len = sizeof(udp_server_buff);
        ...
        while (1) {
            ...
            // Data received
                ...
                // ESP_LOGI(TAG,"=== RECV ===");
                // for (int i = 0; i < len; i++) {
                //     // print each byte received as decimal
                //     ESP_LOGI(TAG,"%d ", udp_server_buff[i]);
                // }
                // ESP_LOGI(TAG,"=== RECV ===");
...
void init_receiver_udp(StreamBufferHandle_t network_stream_buf) {
    ...
    ESP_LOGI(TAG,"Init UDP Server!\n");
    xTaskCreate(udp_server_task, "udp_server_task", 4096, (void*) network_stream_buf, 5, &udp_server_task_handle);
    // confirm that the task is created
    configASSERT(udp_server_task_handle);
}